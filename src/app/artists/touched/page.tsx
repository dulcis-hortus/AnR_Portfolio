  ...
  return (
    <div className="album-scope" style={ALBUM_VARS}>
      ...
      <AlbumSection id="order" no="④" title="러닝오더와 아크">
        <p>
          《역광》의 러닝오더는 선공개된 8곡 + 새로 추가될 4곡(타이틀 2곡
          포함), 총 12곡.
        </p>
      ...
      <AlbumSection id="title" no="⑥" title="타이틀 전략과 퍼널">
        ...
        <h3>팬은 퍼포먼스를 목격하는 순간에 태어난다 — 선공개 전략</h3>
        <ul>
          <li>
            시차 더블 타이틀: Sunburst 선공개(발매 2~3주 전) → Whiteout 앨범
            발매일 공개.
          </li>
          <li>
            Sunburst는 라이브 세션 영상 동반 공개: 실연주 + 멀티트랙 후반
            믹싱(킬링보이스 포맷 — 유입 데이터에 이미 존재하는 검증된 창).
          </li>
        </ul>
        <p>
          라이브 영상에 스튜디오 음원을 싱크하는 안도 검토했으나 기각했다(
          <a href="#ledger">판단의 기록</a>). 이 팀의 유입층은 라이브의
          진위를 판별하며 들어온 팬층이므로 싱크 연출은 진정성 자산을 담보로
          잡는 선택이며, 형상과 실체의 일치를 말하는 《역광》 컨셉과 정면
          충돌한다.
        </p>
        <p>
          선공개 Sunburst가 결핍을 확실히 암시할수록 Whiteout의 부담은
          줄어든다. Sunburst는 신규 유입 장치이자 Whiteout의 서사적 정당성을
          완성하는 곡 — Sunburst가 제 역할을 못 하면 Whiteout도 흔들린다.{' '}
          <strong>리스크 두 가지:</strong> 선공개 부진 시 앨범 전체 기대치
          하락, 홍보 화력이 두 시점으로 분산.
        </p>
        <p className="alb-fn">
          공개 일정(주차 단위)·세션 영상 제작 사양 등 실행 디테일은 별도
          실행 설계 문서로 정리되어 있으며, 협의 시 공유한다.
        </p>
      </AlbumSection>
      ...
      <AlbumSection id="stage" no="⑧" title="무대·비주얼 번역">
        <p>
          ...
          유입이다(post-hoc 재그룹). <Chip kind="int" />
