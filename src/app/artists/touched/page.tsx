    ...
    <div className="album-scope" style={ALBUM_VARS}>
      ...
      <AlbumSection id="diagnosis" no="①" title="진단">
        <ul className="alb-items">
          ...
          <li>
            ...
            <p>
              <Chip kind="obs" />
              스포티파이 월간 청취자 76,291명(2026.07 기준). 동일 씬 내 라이브
              동원 규모가 더 작은 밴드들이 같은 시점 90,000~112,000명대를
              기록 — 라이브 동원력과 스트리밍 지표의 순위가 역전되는
              사례.
            </p>
            <p>
              <Chip kind="int" />
              격차의 일부는 장르 특성(라이브 중심 사운드의 낮은 스트리밍
              전환)으로 설명되나, 최상위 라이브 동원력 대비 청취자 규모의
          ...
          <li>
            ...
            <p>
              ...
              &ldquo;커져가는 감정들은 눈덩이가 되어가.&rdquo;) 구원 서사에서
              일방향 관계 구도가 반복됨(
          ...
          <li>
            ...
            <p>
              ...
              하드록과 팝 사이에서 아직 어느 쪽의 언어로도 완결되지 않은
              지점. 특정
          ...
          <li>
            ...
            <p>
              한 번밖에 없는 서사적 이벤트. 지금의 탐색을 명확한 의도 없이
      ...
      <AlbumSection id="concept" no="③" title="컨셉과 근거">
        <p>
          ...
          <strong>&lsquo;내면의 어둠을 마주할 때 찾아오는 위로&rsquo;</strong>
          이다.
        </p>
        <p>
          그러나 이 주제를 풀어내기 위해 터치드가 먼저 해결해야 할
          과제가 있다. 터치드는 결핍보다 성취에 가까운 서사를 걸어온 팀이다 —
          뚜렷한 개성, 강렬한 퍼포먼스, 짧은 기간의 급격한 성장. 어둠을
          노래하려는 팀에게 정작 &lsquo;결핍의 언어&rsquo;가 익숙하지 않다는 것
          — 이것이 이 앨범이 풀어야 할 첫 번째 문제였다. 끊임없이 빛에 다가가는
          터치드가 가장 선명하게 호명할 수 있는 &lsquo;어둠&rsquo;은 무엇인가.
        </p>
        <p>
          그 답으로 정규 1집 제목 《역광》을 제시한다. 역광의 어둠은 눈부신
          빛에 둘러싸인 사람만이 가질 수 있는 어둠이다. 밴드의 지향점(더 밝은
          곳, 더 큰 무대)을 교정하지 않고 그대로 정렬했다는 점에서 《역광》은{' '}
          <strong>진정성을 확보</strong>하며, 이는 이 컨셉의 가장 큰 이점이다.
        ...
        <p>
          ...
          《역광》은 선언으로 답하지 않는다.{' '}
          <strong>
            정체성은 앨범이 답할 질문이 아니라 날카로운 선택을 하고 나면 결과로
            남는다.
          </strong>{' '}
          명확한 컨셉 아래
          사운드와 발화 선택을 일관되게 좁히면 그 총합이 정체성으로 남는다.
          《역광》은 팬이 시각적 목격으로 태어난다는 유입 분석 결과(
          <a href="#diagnosis">① 진단</a>)와도 정합한다 — 조명 연출·앨범아트로
          직역 가능한 드문 컨셉이다.
        </p>
      </AlbumSection>
      ...
      <AlbumSection id="feasibility" no="⑦" title="실현 가능성">
        <p>
          가상 앨범 기획안은 항상 동일한 질문을 떠올리게 한다 — 이 컨셉은
          실제로 밴드가 수행할 수 있는 방향인가. 이를 보완하기 위해 공개
          인터뷰·기록을 인용해 실현 가능성을 검증했다.
        </p>
        <table className="alb-table">
          <thead>
            <tr>
        ...
        <p>
          이처럼 《역광》의 컨셉은 외부에서 이식한 것이 아니라, 밴드가 이미
          남겨온 말과 기록에서 출발한 하나의 해석이며, 그 방향에 맞는 언어를
          설계해본 시도다.
        </p>
      </AlbumSection>
      ...
      <AlbumSection id="stage" no="⑧" title="무대·비주얼 번역">
        <p>
          유입 언급의 30% 정도가 퍼포먼스 목격임을 감안했을 때(
          ...
          <div className="alb-none" id="not-here">
            ...
            <ul>
              ...
              <li>
                <b>실현 가능성 분석 전체본</b>
                <span>
                  공개 인터뷰·방송 발언에 근거한 앨범 사양의 대응 분석
                </span>
              </li>
            </ul>
          </div>
