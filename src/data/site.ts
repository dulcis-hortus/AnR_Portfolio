...
// 실존 아티스트를 다루는 페이지 색인 차단(2026-08-04). 이력서 링크로 들어온
// 사람에게는 아무 영향이 없고 — 링크도 OG 미리보기도 그대로 작동한다 —
// 아티스트명 검색으로 팬덤·소속사가 유입되는 경로만 닫는다. 약점 진단과
// 미발매곡 목록이 담겨 있어, 채용 이득이 없는 그 경로만 비대칭적으로 위험하다.
// 위험 질의는 '터치드 역광'이 아니라 '터치드 정규 1집'처럼 팬이 실제로 치는 쪽.
//
// 차단: 홈 · 분석 방법 · 아티스트 페이지 5종
//   - 홈은 카드에 아티스트명·앨범명이 있고,
//   - 분석 방법은 팀별 판정표가 있어 노출량이 가장 크다(데이먼스이어·최유리는
//     전용 페이지가 없어 이 페이지에만 등장한다).
// 허용: About · 프로젝트 — 아티스트명은 네비 드롭다운 1회뿐이고, 제목 템플릿에
//   '정지영 — A&R 포트폴리오'가 붙어 본인 이름 검색에는 계속 걸린다.
//
